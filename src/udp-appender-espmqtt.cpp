...
UDPAppenderEspMQTT::UDPAppenderEspMQTT(const char* ipaddr, uint16_t port) : _fd(-1)
{
  Serial.println("Constructor wifi UDPAppenderEspMQTT ") ;
  ...
  if (_addr.sin_addr.s_addr == 0)
    WiFi.onEvent([this](arduino_event_id_t event, arduino_event_info_t info) {
      Serial.println("wifi on WiFi.onEvent "+ event) ;
    switch (event)
    {
...
UDPAppenderEspMQTT::~UDPAppenderEspMQTT()
{
  if (_fd >= 0)
  {
    shutdown(_fd, 2);
...
bool UDPAppenderEspMQTT::append(const LogMessage* message)
{
